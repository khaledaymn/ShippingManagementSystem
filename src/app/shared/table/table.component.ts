...
import { TableConfig, TableData, TableEvent, TableSort, TableColumn, TableAction } from '../../core/models/table';
...
@Component({
  ...
  //     state('*', style({ height: '*' })),
  //     transition('void <=> *', animate('50ms ease')),
  //   ]),
  // ],
})
export class TableComponent implements OnInit, OnDestroy, OnChanges {
  ...
  private setupPageSizeOptions(): void {
    this.pageSizeOptions = this.config?.pageSizeOptions?.length
  ...
  get activeFilterCount(): number {
    ...
    return Object.keys(this.filterForm.value).filter(
