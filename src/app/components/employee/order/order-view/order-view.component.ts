...
export class OrdersViewComponent implements OnInit, OnDestroy {
  ...
  tableConfig: TableConfig = {
    ...
    actions: [
      ...
      },
      {
        ...
        action: 'changeStatus',
        color: 'warning',
        visible: (item: Order) => item.orderState === OrderStateValues.New && this.canEdit
      },
      {
        label: 'Assign to Delivery',
        icon: 'bi bi-truck',
        action: 'assignToDelivery',
        color: 'success',
        visible: (item: Order) => item.orderState === OrderStateValues.Pendding && this.canEdit
      },
      {
        label: 'Delete',
        icon: 'bi bi-trash',
        action: 'delete',
        color: 'danger',
        visible: (item: Order) => item.orderState === OrderStateValues.New && this.canDelete
      }
    ],
    filters: [
      {
      ...
      {
        key: 'shippingRepresentativeId',
        label: 'Shipping Representative',
        type: 'select',
        options: [],
      },
      {
  ...
  private checkPermissions(): void {
    ...
    this.canView = isAdmin || this.authService.hasPermission('Orders', 'View');
    this.canCreate = isAdmin || this.authService.hasPermission('Orders', 'Create');
    this.canEdit = isAdmin || this.authService.hasPermission('Orders', 'Edit');
    this.canDelete = isAdmin || this.authService.hasPermission('Orders', 'Delete');
  ...
  private loadMerchants(): void {
    this.merchantService
      .getMerchants({ pageIndex: 1, pageSize: 1000, isActive: true })
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response) => {
          this.merchants = response.data as Merchant[];
          this.tableConfig.filters!.find((f) => f.key === 'merchantId')!.options = [
        ...
        error: (error) => {
          this.error = 'Failed to load merchants: ' + error.message;
        },
      });
  }
  ...
  onTableEvent(event: TableEvent): void {
    ...
    switch (event.type) {
      ...
      case 'action':
        ...
        break;
      case 'update':
        this.selectedOrder = this.orders.filter((item) => item.id === event.data.id)[0];
  ...
  private handleAction(action: string, order: Order): void {
    ...
    switch (action) {
      ...
      case 'assignToDelivery':
        this.openAssignDeliveryModal(order);
        break;
  ...
  private viewDetails(orderId: number) {
    this.router.navigate(['employee/orders/details', orderId]);
  }
